...
def TCCD(seed_id, params):
    ...
    while True:
        ...
        targetUtil = sUtil * NUMP
        T = np.array([170, 100])
        # T = np.array([100, 100, 100, 100])

        CSW = np.random.randint(30, 100+1,  NUMT)    

        targetUtil - CSW[0]/T[0]

        CSW[1] = np.round((targetUtil - CSW[0]/T[0]) * T[1])
        # CSW[3] = np.round((targetUtil - CSW[0]/T[0] - CSW[1]/T[1] - CSW[2]/T[2]) * T[3])

        if CSW[1] < 30 or CSW[1] > 100:
            new_seed += 1000
            if new_seed >= 2**32:
                new_seed = (new_seed % 2**32)
            continue
        # if CSW[3] < 30 or CSW[3] > 100:
        #     new_seed += 1000
        #     if new_seed >= 2**32:
        #         new_seed = (new_seed % 2**32)
        #     continue
